...
// static inline void setVdacChannelOutput(VDAC_TypeDef *vdac, unsigned int channel, uint16_t value) {
...
//   setVdacChannelOutput(VDAC_SIG_ID, VDAC_SIG_CH, vdacOUT_start);
...
//   setVdacChannelOutput(VDAC_SIG_ID, VDAC_SIG_CH, vdacOUT_value);
...
//   // Skip the first sample after starting a new measurement
//   // This sample may have stale data from before the measurement started
//   if (iadc_isFirstSample) {
      
//       // Drain FIFO to clear any stale data
//       // while (IADC_getScanFifoCnt(IADC0) > 0) {
//       //   (void) IADC_pullScanFifoResult(IADC0);
//       // }
//       iadc_isFirstSample = false;
//       // return;  // Exit early without incrementing counters or processing data
//   } else {
//     // Stop scan immediately to prevent additional triggers
//     iadcSAMPLE_count++; // 0 to 1
...
//   if (flags & 0x1) { // Comp0 match. Trigger IADC sample 18 samples before underflow
...
//   } else if (flags & 0x2) { // Underflow - time to update VDAC output based on operating mode 
// #if RUN_MODE == 0
//           GPIO_PinOutSet(DBG2_OUT_PORT, DBG2_OUT_PIN);
// #endif

//       if (operating_mode == 0) {
//               // Square Wave Voltammetry Mode (original behavior)
//               // Handle VDAC voltage changes in the underflow portion for proper synchronization
//               // Only change pulse/state after we've collected at least one sample
//               // This prevents the initial baseline (stay at start voltage) from being
//               // skipped when the LETIMER underflow occurs immediately after enabling.
//               if ((iadcSAMPLE_count > 0) && (iadcSAMPLE_count % iadcSAMPLESperPULSE) == 0) {
//                 vdacOUT_count++;
...
//   LETIMER_IntClear(LETIMER0, _LETIMER_IF_MASK); // Clear all internal flags
...
// void warmupVdac(void) {
//     setVdacChannelOutput(VDAC_SIG_ID, VDAC_SIG_CH, 0);
//     sl_sleeptimer_delay_millisecond(10); // let reference and output buffer settle
//     setVdacChannelOutput(VDAC_SIG_ID, VDAC_SIG_CH, vdacOUT_ref);
