...
// {
//   // Reset queue and clear packet data on every new measurement
//   BLE_queue_head = 0;
//   BLE_queue_tail = 0;
//   BLE_queue_count = 0;
//   for (int i = 0; i < BLE_QUEUE_SIZE; i++) {
//     for (int j = 0; j < BLE_MAX_PACKET_SIZE; j++) {
//       BLE_packet_queue[i].data[j] = 0;
//     }
//     BLE_packet_queue[i].size = 0;
//   }
//   // Also clear the current packet buffer and reset BLE transmit flags
//   BLE_result_counter = 0;
//   BLE_dropped_packets = 0;
//   BLE_transmission_busy = false;
//   BLE_notify_result = false;
//   for (int k = 0; k < BLE_MAX_PACKET_SIZE; k++) {
//     BLE_current_packet[k] = 0;
//   }
...
//       // Mode-specific initialization (must happen for every measurement)
//       if (operating_mode == 1) {
//           BLE_packetSize = 200;
...
//   // Check if measurement is active before processing. This lets us keep the timer running for bluetooth.
