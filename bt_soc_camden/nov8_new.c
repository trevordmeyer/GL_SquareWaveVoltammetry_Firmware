...
//   // COMP0 (0x1 = bit 0): Triggers PRS -> IADC sampling (handled in hardware)
//   // UF (0x4 = bit 2): Update voltage for next measurement cycle
  
//   if (flags & 0x1) { // COMP0 is (0x1UL << 0), UF is (0x1UL << 2)
//       // COMP0 event - IADC triggered by PRS hardware
//       // Increment sample count here since IADC sampling has been triggered
//       iadcSAMPLE_count++;
...
//   } else {
//       // UF (underflow) event - update voltages for next cycle
// #if RUN_MODE == 0
//           GPIO_PinOutSet(DBG2_OUT_PORT, DBG2_OUT_PIN);
// #endif
...
//   // The IADC is triggered by PRS from LETIMER
...
//   // IADC should have lower priority (higher number) than LETIMER to avoid conflicts
...
// /**************************************************************************//**
//  * @brief
//  *    PRS (Peripheral Reflex System) initialization
//  *    Routes LETIMER0 CH0 (COMP0 event) signal to IADC scan trigger via hardware
//  *    This eliminates software timing jitter for precise sampling
//  *    
//  *    Note: LETIMER0 only provides CH0/CH1 PRS signals (compare events), not UF
//  *    Therefore we trigger IADC on COMP0, and handle voltage changes on UF
//  *****************************************************************************/
// void initPRS(void) {
//  // Use LETIMER0 CH0 (COMP0) as async PRS to trigger IADC in EM2
//  CMU_ClockEnable(cmuClock_PRS, true);
